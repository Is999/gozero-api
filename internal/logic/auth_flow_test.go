...
func TestAuthMainFlowIntegration(t *testing.T) {
	...
	profile := requireUserProfile(t, NewUserLogic(profileCtx, svcCtx).Profile(), codes.FetchSuccess)
...
func newAuthFlowTestService(t *testing.T) (*svc.ServiceContext, redis.UniversalClient, *[]collector.Event) {
	t.Helper()
	...
	manager, err := collector.New(cfg.Collector, client)
	if err != nil {
		t.Fatalf("collector.New() error = %v", err)
	}
	seen := make([]collector.Event, 0, 4)
...
func requireAuthFlowEvents(t *testing.T, events []collector.Event, wants []authFlowEventWant) {
	t.Helper()
