...
func TestTypedSysConfigRejectsKeyTypeMismatch(t *testing.T) {
	logicObj := NewSysConfigLogic(context.Background(), svc.NewServiceContext(config.Config{}, "v1", svc.Dependencies{}))
...
func newSysConfigLogicForKeyTest(client redis.UniversalClient) *SysConfigLogic {
	return NewSysConfigLogic(context.Background(), svc.NewServiceContext(config.Config{AppID: "site-a"}, "v1", svc.Dependencies{Rds: client}))
}
