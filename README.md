...
`api` 是面向前台用户项目的 HTTP API 服务。后台管理与运营任务由 `admin` 承担，本服务只保留前台交互所需的结构性核心能力：
...
发布模板位于 `deploy/`，包括 Dockerfile、systemd 单元和集成依赖 Compose；Prometheus 告警规则位于 `docs/prometheus/api-alerts.yml`。
...
- 认证风控事件：注册、登录、刷新、退出、登录态鉴权失败、限流和用户级 session 失效会投递 `auth.security` 脱敏事件；默认 Processor 会统计 `api_auth_security_events_total{app_id,action,reason,category}`，Collector 关闭不影响接口响应。
